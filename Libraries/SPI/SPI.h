...
public:
  ...
  uint8_t ICACHE_RAM_ATTR transfer(uint8_t data);
  uint8_t ICACHE_RAM_ATTR transferFast(uint8_t data);
  uint16_t transfer16(uint16_t data);
  void ICACHE_RAM_ATTR transfer(void *buf, uint16_t count);
...
private:
  ...
  void ICACHE_RAM_ATTR setDataBits(uint16_t bits);
};
